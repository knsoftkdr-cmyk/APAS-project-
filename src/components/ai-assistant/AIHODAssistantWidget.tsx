...
// ANSWERING LOGIC
// ---------------------------------------------------------------------
// Calls the `hod-assistant` Supabase edge function, which derives the
// caller's identity from their auth JWT server-side (never from anything
// passed here) and answers using ONLY that HOD's own school's data -
// Reports, Communication, Surveys, Academic Calendar, Timetable,
// Syllabus Coverage, Competency Heatmap, and Competency Definitions.
// See supabase/functions/hod-assistant/index.ts for the full
// implementation.
async function getAssistantReply(userText: string, recentHistory: ChatMessage[]): Promise<string> {
  const { data, error } = await supabase.functions.invoke("hod-assistant", {
    body: {
      message: userText,
...
export function AIHODAssistantWidget() {
  ...
  const orbClasses: Record<VoiceState, string> = {
    idle: "from-violet-400 via-purple-400 to-fuchsia-400 animate-pulse",
    listening: "from-violet-400 via-fuchsia-300 to-purple-400 animate-pulse",
    thinking: "from-purple-500 via-violet-400 to-fuchsia-500 animate-spin",
    speaking: "from-fuchsia-300 via-purple-200 to-violet-300 animate-pulse",
  };
    ...
    <>
      {!open && (
        <button
          ...
          className="fixed bottom-5 right-5 z-50 flex h-14 w-14 items-center justify-center rounded-full bg-violet-600 text-white shadow-lg hover:bg-violet-700 transition-colors"
          aria-label="Open HOD Assistant"
        >
        ...
        <div className="fixed inset-0 z-[60] flex flex-col items-center justify-center bg-black">
          ...
          <div className="relative flex h-56 w-56 items-center justify-center">
            ...
            {voiceState === "listening" && (
              <div className="absolute inset-1 rounded-full border-2 border-fuchsia-300/60 animate-pulse" />
            )}
            {voiceState === "thinking" && (
              <div className="absolute inset-1 rounded-full border-2 border-t-transparent border-violet-300/70 animate-spin" />
            )}
            {voiceState === "speaking" && (
              <div
        ...
        <Card className="fixed inset-x-3 top-16 bottom-3 z-50 flex flex-col shadow-2xl sm:inset-x-auto sm:top-auto sm:bottom-5 sm:right-5 sm:h-[520px] sm:w-[360px]">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 border-b py-3">
            <CardTitle className="flex items-center gap-2 text-sm">
              <Sparkles className="h-4 w-4 text-violet-600" />  APAS Agent
            </CardTitle>
            <div className="flex items-center gap-2">
              {voiceSupported && ttsSupported && (
                <button
                  onClick={enterVoiceMode}
                  className="flex h-8 w-8 items-center justify-center rounded-full bg-violet-600 text-white shadow hover:bg-violet-700 transition-colors shrink-0"
              ...
              <div className="flex flex-col gap-3">
                {messages.length === 0 && (
                  <p className="text-xs text-muted-foreground">
                    Hi! I'm your HOD Assistant. Ask me about Reports, Communication, Surveys, the Academic Calendar, Timetable, Syllabus Coverage, or Competencies - or tap the purple icon above for hands-free voice mode.
                  </p>
                )}
                {messages.map((m, i) => (
                  <div key={i} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
                    <div className={`max-w-[85%] rounded-lg px-3 py-2 text-xs ${m.role === "user" ? "bg-violet-600 text-white" : "bg-muted"}`}>
                      <p>{m.text}</p>
                    </div>
                  </div>
                ))}
