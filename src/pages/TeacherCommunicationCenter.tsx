    ...
    <div className="rounded-2xl p-5 md:p-6 mb-4 relative overflow-hidden bg-gradient-to-r from-green-600 to-emerald-600 shadow-lg shrink-0">
      ...
      <div className="relative flex items-center gap-3 md:gap-4">
        ...
        <div>
          ...
          <p className="text-green-100 text-xs md:text-sm mt-0.5">Message parents, students, and admins in one place</p>
        </div>
      </div>
    </div>
  )}

  <div className={`flex justify-center sm:justify-end mb-4 shrink-0 ${view === "communities" ? "mt-2" : ""}`}>
    <div className="flex gap-1 bg-slate-100 p-1 rounded-full">
      <button
        onClick={() => setView("messages")}
        className={`px-4 py-1.5 rounded-full text-sm font-medium transition-all ${
          view === "messages" ? "bg-white text-green-700 shadow-sm" : "text-slate-500 hover:text-slate-700"
        }`}
      >
        Messages
      </button>
      <button
        ...
        className={`px-4 py-1.5 rounded-full text-sm font-medium transition-all ${
          view === "communities" ? "bg-white text-indigo-700 shadow-sm" : "text-slate-500 hover:text-slate-700"
        }`}
      >
        Teacher Communities
      </button>
            ...
            <div className={`border-r border-slate-100 bg-white flex flex-col ${showMobileChat ? "hidden md:flex" : "flex"}`}>
              ...
              <Tabs value={contactTab} onValueChange={(v: any) => setContactTab(v)} className="px-3 pt-3">
                <TabsList className="grid grid-cols-3 w-full h-9 bg-slate-100 rounded-full p-1">
                  ...
                  <TabsTrigger value="admin" className="text-xs gap-1 rounded-full data-[state=active]:bg-green-600 data-[state=active]:text-white data-[state=active]:shadow-sm">
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="min-w-0 flex-1">
                            <p className="text-sm font-semibold truncate text-slate-800">{g.name}</p>
                            <p className="text-xs text-muted-foreground truncate">{g.subtitle}</p>
                          </div>
                        </button>
                      );
                    })}
              ...
              {!selectedContact ? (
               <div className="flex-1 flex flex-col items-center justify-center text-center p-6 bg-slate-50/50">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    Your conversations with parents, students, and admins will appear here.
                  </p>
                </div>
              ) : (
                <>
                  ...
                  <div className="p-3 border-b border-slate-100 flex items-center gap-2.5 bg-gradient-to-r from-green-600 to-green-500">
                    ...
                    <div className="h-10 w-10 rounded-full flex items-center justify-center shrink-0 text-xs font-bold bg-white/20 text-white backdrop-blur-sm">
                      {selectedContact.kind === "class_group" ? <Users className="h-4.5 w-4.5" /> : selectedContact.name[0]}
                    </div>
                    <div className="min-w-0 flex-1">
                    ...
                    {selectedContact.role === "parent" && selectedContact.kind === "individual" && (
                      <Button
                        size="sm"
                        variant="outline"
                        className="gap-1.5 text-xs bg-white/10 border-white/30 text-white hover:bg-white/20 hover:text-white"
