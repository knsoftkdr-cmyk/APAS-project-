...
const TeacherPanel = () => {
  ...
  const { data: assessments, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: submittedAssessments, error: assessError } = await supabase
        .from("student_assessments")
        .select("id, submitted_by, student_name, student_age, age_group, responses, created_at, student_class, section")
        .eq("school_id", profile.school_id)
      ...
      const merged = students.map((student) => {
        const assessment = (submittedAssessments || []).find(
          (a) => a.submitted_by === student.id
        );
        return {
          id: assessment?.id ?? student.id,
          student_name: student.full_name,
          student_age: assessment?.student_age ?? null,
          age_group: assessment?.age_group ?? null,
          responses: assessment?.responses ?? null,
          created_at: assessment?.created_at ?? null,
          student_class: assessment?.student_class ?? student.class_grade ?? null,
          section: assessment?.section ?? student.section ?? null,
          submitted_by: student.id,
          hasAssessment: !!assessment,
        };
      });

      return merged as StudentAssessment[];
    },
    enabled: !!user?.id && !!profile?.role && !!profile?.school_id,
  });
...
<div className="hidden md:block">
          ...
          <img
            ...
            className="hidden md:block absolute right-0 bottom-3 w-[180px]"
          />
        </div>
